...
export class DraftsService {
  ...
  async generateDraft(dto: GenerateDraftDto, organizationId: string): Promise<{ markdown: string }> {
    this.logger.log(`Generating draft for template: "${dto.template}" (Org: ${organizationId})`);

    let context = '';
    
    // Fetch context from sections if specified
...
Begin the markdown draft:`;
    ...
    // Clean up response of markdown fences if LLM wrapped it in ```markdown
  ...
  async refineDraft(dto: RefineDraftDto, organizationId: string): Promise<{ markdown: string }> {
